...
class Code(object):
    ...
    def __gene_line(cls, draw, width, height):
        ...
        draw.line([begin, end], width=2)

    # 生成随机生成数字或字母
    def random_hexdigits(self, len=1):
        a = random.sample(string.hexdigits, len)
        return a
    ...
    def creat_code(self, width=80, height=24, color=(192, 192, 192)):
        ...
        font = ImageFont.truetype(os.path.join(STATICPATH, 'fonts/Lora-Regular.ttf'), 20)
        draw = ImageDraw.Draw(image)
        self.fill_color(draw, image, 5)  # 填充背景色
        self.fill_dischar(draw, image, 10)  # 填充干扰
        code = self.fill_char(draw, image, 5, 1, font)  # 填充验证码
